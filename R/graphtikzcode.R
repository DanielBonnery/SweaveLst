graphtikzcode <-
function(texte,widthe=7,heighte=7,scale=c(1,1),yxratio=c(1,1),caption=NULL,label=NULL,addfigureenv=FALSE,sanitize=FALSE,...){
  par()
  ...
  dev.off()  
  return(removetikzfile(tmpfile,scale=scale,yxratio=yxratio,caption=caption,label=label,addfigureenv=addfigureenv))}
